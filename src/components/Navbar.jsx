...
export default function Navbar({ active }) {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'nav-glass shadow-sm' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-6 h-[74px] flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a
              ...
              className={`font-body text-[14px] font-semibold transition-colors ${scrolled ? 'text-gray-500' : 'text-white/60'} hover:text-[#e8006f]`}
            >
              +91 90925 51753
            </a>
            <button onClick={() => handleNav('contact')} className="btnp text-[14px] px-5 py-2.5 rounded-full">
              Get Quote
            </button>
          </div>

          {/* Hamburger */}
          <button className="lg:hidden flex flex-col gap-[5px] p-2" onClick={() => setOpen((v) => !v)} aria-label="Toggle menu">
            {[0, 1, 2].map((i) => (
              <span key={i} className={`block w-6 h-0.5 transition-all duration-300 ${scrolled ? 'bg-[#0c0c14]' : 'bg-white'}
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-white flex flex-col items-center justify-center gap-7">
          ...
          <button onClick={() => handleNav('contact')} className="btnp px-8 py-3.5 rounded-full text-base mt-2">
            Get Quote ✨
          </button>
        </div>
      )}
